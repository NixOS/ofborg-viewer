...
class Log {
  constructor(name, metadata = null, { label = null } = {}) {
    ...
    this.$tab = html(
      `<li><label><input type="radio" name="selected_tab"><span></span></label></li>`,
    )[0];
  ...
  backlog(lines, log_url = null) {
    ...
    lines.slice(start, lines.length).forEach((text) => {
    ...
    const $link = html(
      `<a class="truncated">Log has been truncated... (${length} lines shown out of ${lines.length}.)</a>`,
    )[0];
    if (log_url) {
      $link.href = log_url;
    }
